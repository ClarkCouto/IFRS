...
public class TelaFuncionario {
	...
	public FuncionarioAbstrato setSexo(FuncionarioAbstrato f){
		...
		do{
			...
			if(sexo == 'M'){
				f.setSexo(f.MASCULINO);
				valido = true;
			}
			else if(sexo == 'F'){
				f.setSexo(f.FEMININO);
				valido = true;
			}
			else {
	...
	public FuncionarioAbstrato setLimiteINSS(FuncionarioAbstrato f){
		double limiteINSS;
		boolean valido = false;
		do{
			System.out.print("Digite o novo limite do INSS: ");				
			limiteINSS = scan.nextDouble();
			valido = f.setLimiteINSS(limiteINSS);
			if (!valido){ 
